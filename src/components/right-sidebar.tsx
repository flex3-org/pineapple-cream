...
import { Link2, Hash, Calendar, FileText, TrendingUp, Clock } from "lucide-react"
...
export function RightSidebar({ activeNote, allNotes }: RightSidebarProps) {
  const insights = useMemo(() => {
    ...
    const unlinkedMentions = allNotes.filter((note) => {
      ...
      const sharedTags = note.tags.filter((tag) => activeNote.tags.includes(tag))
    ...
    const daysSinceCreated = Math.floor((new Date().getTime() - activeNote.createdAt.getTime()) / (1000 * 60 * 60 * 24))
    const daysSinceModified = Math.floor(
      (new Date().getTime() - activeNote.updatedAt.getTime()) / (1000 * 60 * 60 * 24),
    )
    ...
    return (
      <div className="w-80 bg-sidebar border-l border-sidebar-border flex flex-col">
        <div className="p-4 border-b border-sidebar-border">
          <h3 className="text-sm font-medium text-sidebar-foreground">Insights</h3>
        </div>
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-sidebar-accent rounded-lg p-3">
                ...
                <p className="text-lg font-semibold text-sidebar-foreground mt-1">{insights.wordCount}</p>
              </div>
              <div className="bg-sidebar-accent rounded-lg p-3">
                <div className="flex items-center gap-2">
                ...
                <p className="text-lg font-semibold text-sidebar-foreground mt-1">{insights.readingTime}m</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Linked mentions</h4>
              <span className="text-xs text-muted-foreground">{insights.linkedMentions.length}</span>
            </div>
            {insights.linkedMentions.length > 0 ? (
              <div className="space-y-2">
                {insights.linkedMentions.map((note) => (
                  <Button key={note.id} variant="ghost" size="sm" className="w-full justify-start h-auto p-2 text-left">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-sidebar-foreground truncate">{note.title}</p>
                      <p className="text-xs text-muted-foreground mt-1">{note.updatedAt.toLocaleDateString()}</p>
                    </div>
                  </Button>
                ))}
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Unlinked mentions</h4>
              <span className="text-xs text-muted-foreground">{insights.unlinkedMentions.length}</span>
            </div>
          ...
          {insights.links.length > 0 && (
            <>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">External links</h4>
                  <span className="text-xs text-muted-foreground">{insights.links.length}</span>
                </div>
                <div className="space-y-2">
