...
export function LeftSidebar({ notes, folders, onNoteSelect }: LeftSidebarProps) {
  ...
  const getUnorganizedNotes = () => {
    const organizedNoteIds = new Set(folders.flatMap((folder) => folder.children))
          ...
          {folders.map((folder) => (
            <div key={folder.id} className="mb-2">
              ...
              >
                ...
                <span className="ml-auto text-xs text-muted-foreground">{getNotesInFolder(folder.id).length}</span>
              </Button>
                ...
                <div className="ml-4 mt-1 space-y-1">
                  {getNotesInFolder(folder.id).map((note) => (
                    ...
                    >
                      ...
                      <span className="text-sm truncate flex-1 text-left">{note.title}</span>
          ...
          {getUnorganizedNotes().length > 0 && (
            <div className="mb-2">
              <div className="px-2 py-1">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Other Notes</span>
              </div>
              ...
              {Array.from(new Set(notes.flatMap((note) => note.tags)))
                ...
                .map((tag) => (
                  ...
                  >
                    ...
                    <span className="text-sm truncate flex-1 text-left">{tag}</span>
                    <span className="text-xs text-muted-foreground ml-2 flex-shrink-0">
